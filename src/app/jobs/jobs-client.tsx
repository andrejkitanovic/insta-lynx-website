...
export default function JobsClient({ initialJobs }: { initialJobs: ApiJob[] }) {
  return (
    <Suspense>
      <JobsPageContent initialJobs={initialJobs} />
    </Suspense>
  );
}

function JobsPageContent({ initialJobs }: { initialJobs: ApiJob[] }) {
  ...
  const skipNextSortFetch = useRef(true);

  useEffect(() => {
    if (skipNextSortFetch.current) {
      skipNextSortFetch.current = false;
      return;
    }
  ...
  return (
    <>
      ...
      <section className="relative isolate overflow-hidden border-b border-white/8">
        <PhotoBackdrop src="/images/truck-motion.jpg" position="center 65%" priority />
